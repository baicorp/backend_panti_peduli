...
export const getProfile = (req, res) => {
    ...
    db.query(sql, (err, data) => {
        ...
        return !err ? res.status(200).json(data) : res.status(500).json({ error: err.message });
    });
};
...
export const getProfileById = (req, res) => {
    ...
    db.query(sql, (err, data) => {
        ...
        return data.length > 0 ? res.status(200).json(data) :
            res.status(404).json({ message: 'No profile found with the given ID.' });
    }); 
};
...
export const addNewProfile = (req, res) => {
    ...
    const imgErrorMsg = (!req.files || Object.keys(req.files).length === 0) ? 'No files were uploaded.' : null;
...
export const deleteProfile = (req, res) => {
    ...
    db.query(sql, (err, data) => {
        if (data.affectedRows === 0) {
            return res.status(404).json({ message: 'No profile found with the given ID.' });
